...
const FormCard = () => {
  ...
  /* Данный код содержит обработчики событий и useEffect хук, связанные с валидацией полей формы и установкой состояния `formValid`. */
...
/* Внутри этой функции создается JSON-объект `jsonObj` с значениями из состояний `userName`, `password`, `textInput` и `selectValue`. 
Затем проверяется `formValid`, чтобы определить, является ли форма действительной (все поля валидны). Если форма валидна, выводится алерт с JSON-представлением `jsonObj`. */
...
const handleBlur = (e) => {
  ...
  /*  Это может помочь при валидации формы, чтобы отображать сообщения об ошибках только после того, как поле было изменено. */
